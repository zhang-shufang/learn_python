...
class AlienInvasion:
    ...
    def _upgrade_bullets(self):
        ...
        # for bullet in self.bullets_alien.copy():
    ...
    def _check_bullet_alien_collisions(self):
        ...
        collisions = pygame.sprite.groupcollide(
            self.bullets_ship, self.aliens, True, True
        )
    ...
    def _fire_bullet(self, camp = '', p_start = (0,1), p_target = (0,0)):
        """创建一个子弹，并让其加入编组bullets"""
        print(camp)
        if camp == 'ship':
            if len(self.bullets_ship) < self.settings.buttets_allow:
                new_bullet = Bullet(self, camp, p_start, p_target)
                self.bullets_ship.add(new_bullet)
        elif camp == 'alien':
            new_bullet = Bullet(self, camp, p_start, p_target)
            self.bullets_alien.add(new_bullet)
            print(self.bullets_alien)
    ...
    def _change_fleet_direction(self):
        ...
        numbers = []
        number_aliens = len(self.aliens) - 1
        if number_aliens >= 5:
            number_aliens = 5
        aliens_list = self.aliens.sprites()
        # 从剩下的飞船中随机一定数量的飞船发射子弹。
        # 从该数量中随机编号发射飞船。
        for i in range(random.randint(0, number_aliens)):  
            numbers.append(random.randint(0, number_aliens))

        # print (f"numbers = {numbers}")
        # print(f"len = {len(aliens_list)}") 

        for number in numbers:
            # print (f"number = {number}")
            alien = aliens_list[number]
            self._fire_bullet('alien', alien.rect.midbottom, self.ship.rect.center)
    ...
    def _upgrade_aliens(self):
        ...
        if pygame.sprite.spritecollideany(self.ship, self.aliens):
            self._ship_hit()
        elif pygame.sprite.spritecollideany(self.ship, self.bullets_alien):
            self._ship_hit()
    ...
    def _check_keydown_events(self, event):
        ...
        if event.key == pygame.K_RIGHT:
            ...
            p_target = (self.ship.rect.midtop[0],
                        self.ship.rect.midtop[1] - 1)
            self._fire_bullet('ship', self.ship.rect.midtop, p_target)
    ...
    def _upgrade_screen(self):
        ...
        for bullet in self.bullets_ship:
            bullet.draw_bullet()
